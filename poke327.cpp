...
world_t world;
vector<pokemon_moves> pm_arr;
vector<pokemon_moves> pm_arr_end;
vector<moves> movesArr;
vector<pokemon> pokeArr;
vector<pokemon_types> pt_arr;
...
void parseM(string src) {
    ...
    while(getline(file, line)) {
        ...
        pokeArr.push_back(poke);
        
        line = "";
    }
    
}
void parseMv(string src) {
    ifstream file;
    string line = "";
    string int_string = "";
    ...
    while (getline(file, line)) {
        ...
        movesArr.push_back(move);
        line = "";
    }

}

void parsePm(string src) {
    ifstream file;
    string line = "";
    string int_string = "";
    int pokemon_id, version_group_id, move_id, pokemon_move_method_id, level, order;
    
    src.append("pokemon_moves.csv");
    file.open(src);
    ...
    while(getline(file, line)) {
        ...
        if(version_group_id == 18) {
            ...
            pm_arr.push_back(poke_moves);
        }
        if(version_group_id == 20) {
            pokemon_moves poke_moves(pokemon_id, version_group_id, move_id, pokemon_move_method_id, level, order);
            pm_arr_end.push_back(poke_moves);
        }
...
void parse_poke_species(string src) {
    ifstream file;
    string line = "";
    string int_string = "";
    int id, generation_id, evolves_from_species_id, evolution_chain_id, color_id, shape_id, habitat_id, gender_rate, capture_rate, base_happiness, is_baby, hatch_counter,
    ...
    src.append("/pokemon_species.csv");
    file.open(src);
...
void parse_experience(string src) {
    ifstream file;
    string line = "";
    string int_string = "";
    int growth_rate, level, exp;
...
void parse_type_names(string src) {
    ifstream file;
    string line = "";
    string int_string = "";
    int type_id, language_id;
...
void parsePs(string src) {
    ifstream file;
    string line = "";
    string int_string = "";
    int pokemon_id, stat_id, base_stat, effort;
    
    src.append("pokemon_stats.csv");
    file.open(src);
...
void parsePt(string src) {
    ifstream file;
    string line = "";
    string int_string = "";
    int pokemon_id, type_id, slot;
    
    src.append("pokemon_types.csv");
    file.open(src);
    ...
    while(getline(file, line)) {
        ...
        pt_arr.push_back(poke_types);
        
        line = "";
    }
}

bool open_file(string src) {
    ifstream file;
...
int main(int argc, char *argv[]) {
    ...
    src = "/share/cs327/pokedex/pokedex/data/csv/";
    open = open_file(src);
    
    if(!open) {
        src = getenv("HOME");
        src.append("/.poke327/pokedex/pokedex/data/csv/");
        open = open_file(src);
    }
    
    if(!open) {
        src = "$";
        src.append(getenv("HOME"));
        src.append("/.poke327/pokedex/pokedex/data/csv/");
        open = open_file(src);
    }
